...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (int j = ymin; j < ymax; j++)
    {
        for (int i = xmin; i < xmax; i++)
        {
            ...
            {
                auto [alpha, beta, gamma] = computeBarycentric2D(i, j, t.v);                                               // t.v是三角形三个顶点
                float w_reciprocal = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());                 //计算1/w
                float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                z_interpolated *= w_reciprocal;
                if (z_interpolated < depth_buf[get_index(i, j)])
                {
                    //TODO interpolate normal vector, color and texture color
                    ...
                    set_depth(Vector3f(i, j, z_interpolated));
